...
$\ket 0\ket{\text{stab}} + \ket 1 \ket{\text{IP}}$\\
where $\braket{\vec x \vec y | IP} = \braket{\vec x |\vec y}$


$\ket 0\ket{\text{stab}} + \ket 1 \ket{\text{sum}}$\\
where
$\ket{\text{sum}} \defn \ket{+}^{\otimes n} + \ket{T}^{\otimes n}$\\
where 
$\ket T \defn \ket 0 + e^{i\pi/4} \ket 1$
